...
@WebServlet("/calc.do") /*hello.do로 요청이 오면 현재 서블릿 실행  */
public class CalcServlet extends HttpServlet  { /*servlet에 정의된 메소드,*/ 
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String xvalue = req.getParameter("x"); //파라미터 이름이 "id"인 파라미터의 값 읽기.
		String yvalue = req.getParameter("y"); //파라미터 이름이 "id"인 파라미터의 값 읽기.
		double x = Integer.parseInt(xvalue);
		double y = Integer.parseInt(yvalue);
		String o =" "; //화면에 출력할 연산결과를 저장하는 변수 
		double result = 0;		
		...
		out.println("	<h1> " + x+o+y+"="+result+" </h1> ");
