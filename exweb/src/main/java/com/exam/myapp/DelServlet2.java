...
@WebServlet("/del.do") /*hello.do로 요청이 오면 현재 서블릿 실행  */
public class DelServlet2 extends HttpServlet  { /*servlet에 정의된 메소드,*/ 
...
//		session.setAttribute("user", null);//idVal값을 "user"라는 이름으로 세션객체어 저장 		
//		session.removeAttribute("user");//idVal값을 "user"라는 이름으로 세션객체어 저장 
		session.invalidate();//세션객체 자체를 삭제(모든 속성제거)
		
		ServletContext context = getServletContext(); //현재 웹앱의 서블릿컨텍스트 가져오기 
//     	context.setAttribute("user", null);//idVal값을 "user"라는 이름으로 서블릿객체에 저장 
     	context.removeAttribute("user");//idVal값을 "user"라는 이름으로 서블릿객체에 저장 
		
		Cookie c = new Cookie("user", ""); // 쿠키이름 , 값을 담은 쿠키 객체 생성
		c.setMaxAge(0); //브라우제에서 해당 쿠키를 즉시 삭제
		resp.addCookie(c);//웹브라우저에 저장할 쿠키정보를 응답에 ... 
